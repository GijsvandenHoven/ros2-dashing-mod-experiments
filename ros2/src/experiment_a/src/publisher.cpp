...
public:
  ...
  : Node("basic_bench_publisher"), id_(0), rate_ms(10ms), nanosecond_time_message_expected(0) {
    ...
    publisher_options.message_tracker_opts.tracker_option = rclcpp::MessageTrackerEnum::NONE;
    publisher_options.message_tracker_opts.tracker_result_writing_option = rclcpp::MeasurementWriterEnum::NONE;
    publisher_ = this->create_publisher<basic_bench_msgs::msg::Bench>("bench", BASIC_BENCH_DATA_BUF, publisher_options);

    auto timer_options = rclcpp::TimerOptions();
    timer_options.jitter_tracking_options.jitter_tracker_option = rclcpp::JitterTrackerEnum::NONE;
    timer_options.jitter_tracking_options.result_writer_option = rclcpp::MeasurementWriterEnum::NONE; // why the hell is this inconsistent with PublisherOptions.
